    ...
    <>
      {
        !auth.user ? (<nav className='navbar inset-x-0 md:px-4 fixed top-0 opacity-70 shadow-lg' >
          ...
          <ul className={show ? 'nav_links_mobile' : 'nav_links '} onClick={() => setShow(false)}>
            ...
            <Link to="/register" className='createblogs no-underline text-white hover:bg-gray-400 transition-all duration-500'><li >Register</li></Link>
            <Link to="/login" className='createblogs no-underline text-white  hover:bg-gray-400 transition-all duration-500'><li >Login</li></Link>
          ...
          <h3 className='logo flex ml-2'><BsFillCartCheckFill className='mt-1 mr-1  hover:bg-gray-400 transition-all duration-500'/>Buy Me</h3>

          <ul className={show ? 'nav_links_mobile' : 'nav_links '} onClick={() => setShow(false)}>
        
            <Link to="/" className='blogs no-underline text-white mt-1  hover:bg-gray-400 transition-all duration-500'> <li>Home</li></Link>
       ...
       {/* <Link to="/category" className='createblogs no-underline text-white'><li >Category</li></Link> */}
          ...
          <span onClick={handleToggle} className='rounded-lg p-2 mb-2 m-2 flex  hover:bg-gray-400 transition-all duration-500'>{auth?.user?.name }<AiFillCaretDown className='mt-1'/></span>
         {toggle && 
         ...
         <li> <Link className=' no-underline block px-4 py-2' to={`/dashboard/${auth?.user?.role===1?'admin':'user'}`}> DashBoard</Link> </li>
          <li> <Link className=' no-underline  block px-4 py-2  hover:bg-gray-400 transition-all duration-500' to="/login" onClick={handleLogout}>Logout</Link></li>
          </ul>
          </div>
      }
